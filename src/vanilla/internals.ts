...
type BuildingBlocks = [
  ...
  obsolete_ensureAtomState: undefined, //                      11
...
type BuildingBlockContext = readonly [Readonly<BuildingBlocks>, Store]
...
function ensureAtomState<Value>(
  atom: Atom<Value>,
...
const BUILDING_BLOCK_recomputeInvalidatedAtoms: RecomputeInvalidatedAtoms = (
  ...
  for (const atom of changedAtoms) {
    ...
    stackStates.push(
      ensureAtomState(atom, atomStateMap, storeHooks, atomOnInit, store),
    )
  }
  while (stackAtoms.length) {
    ...
    for (const d of getMountedOrPendingDependents(a, aState, mountedMap)) {
      if (!visiting.has(d)) {
        stackAtoms.push(d)
        stackStates.push(
          ensureAtomState(d, atomStateMap, storeHooks, atomOnInit, store),
        )
      }
    }
  }
...
const BUILDING_BLOCK_readAtomState: ReadAtomState = (ctx, atom) => {
  ...
  const atomState = ensureAtomState(
    atom,
    atomStateMap,
    storeHooks,
    atomOnInit,
  ...
  if (isPromiseLike(valueOrPromise)) {
    for (const a of atomState.d.keys()) {
      addPendingPromiseToDependency(
        atom,
        valueOrPromise,
        ensureAtomState(a, atomStateMap, storeHooks, atomOnInit, store),
      )
    }
  }
...
function buildStore(...partialBuildingBlocks: Partial<BuildingBlocks>): Store {
  const store: Store = {
    get(atom) {
      return storeGet(ctx, atom)
    },
    set(atom, ...args) {
      return storeSet(ctx, atom, ...args)
    },
    sub(atom, listener) {
      return storeSub(ctx, atom, listener)
    },
  }
  ...
  const ctx: BuildingBlockContext = [buildingBlocks, store]
  return store
}
