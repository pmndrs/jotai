...
const ensureAtomState: EnsureAtomState = (store, atom) => {
  const buildingBlocks = getBuildingBlocks(buildingBlockMap, store)
...
const setAtomStateValueOrPromise: SetAtomStateValueOrPromise = (
  ...
  const ensureAtomState = getBuildingBlocks(buildingBlockMap, store)[11]
  const atomState = ensureAtomState(store, atom)
...
const storeGet: StoreGet = (store, atom) => {
  const readAtomState = getBuildingBlocks(buildingBlockMap, store)[14]
...
const externalBuildingBlockMap = new WeakMap<Store, Readonly<BuildingBlocks>>()
const buildingBlockMap = new WeakMap<Store, Readonly<BuildingBlocks>>()

const getBuildingBlocks = (
  buildingBlocksMap: WeakMap<Store, Readonly<BuildingBlocks>>,
  store: Store,
) => {
  const buildingBlocks = buildingBlocksMap.get(store)
...
const getExternalBuildingBlocks = (store: Store): Readonly<BuildingBlocks> =>
...
function buildStore(
  ...
  const store = {
    get(atom) {
      const storeGet = getBuildingBlocks(buildingBlockMap, store)[21]
      return storeGet(store, atom)
    },
    set(atom, ...args) {
      const storeSet = getBuildingBlocks(buildingBlockMap, store)[22]
      return storeSet(store, atom, ...args)
    },
    sub(atom, listener) {
      const storeSub = getBuildingBlocks(buildingBlockMap, store)[23]
