...
class WrappedError extends Error {
  static from(error: unknown) {
    return new WrappedError(undefined, { cause: error })
  }
}
...
export function atomWithObservable<Data>(
  ...
  const observableResultAtom = atom((get) => {
    ...
    const resultAtom = atom<Data | Promise<Data> | WrappedError>(initialResult)

    let preMountData: Data | typeof EMPTY = EMPTY

    let setResult:
      | ((result: Data | Promise<Data> | WrappedError) => void)
      | undefined
    ...
    const startSubscription = () => {
      ...
      subscription = observable.subscribe({
        ...
        error: (error) => {
          reject(error)
          setResult?.(WrappedError.from(error))
        },
        complete: () => {},
      })

      if (!isMounted() && options?.unstable_timeout !== undefined) {
        timer = setTimeout(() => {
          if (subscription) {
            subscription.unsubscribe()
  ...
  return atom(
    (get) => {
      const unwrapResult = (result: unknown) => {
