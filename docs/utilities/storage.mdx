...
#### Working with AsyncStorage (since v2.2.0)

When using AsyncStorage (or any asynchronous storage implementation) with Jotai, there are important differences compared to synchronous storage like localStorage:

1. **Asynchronous Value Access**: The atom value becomes a Promise, which means you need to handle it asynchronously.

2. **Updating Based on Current Value**: When updating the atom and referencing its current value, you'll need to `await` the Promise:
...
const Counter = () => {
  ...
  // Simple increment without referencing current value
  const incrementDirect = () => {
    setCount(1) // Directly sets value to 1, no need to await
  }
  
  // Increment that needs current value
  const increment = () => {
    setCount(async (promiseOrValue) => {
      const currentValue = await promiseOrValue
      return currentValue + 1
    })
  }
  
  // Error handling example
  const incrementWithErrorHandling = async () => {
    try {
      setCount(async (promiseOrValue) => {
        try {
          const currentValue = await promiseOrValue
          ...
          console.error("Error accessing current value:", error)
  ...
  // Using the value in UI requires handling the Promise
...
3. **Error Handling**: When working with AsyncStorage, it's important to handle potential errors:

```jsx
const robustCountAtom = atomWithStorage('robust-count', 0, storage)

const RobustCounter = () => {
  const [count, setCount] = useAtom(robustCountAtom)
  const [error, setError] = useState(null)
  
  // Using loadable to handle async state more elegantly
  const [countState] = useAtom(loadable(robustCountAtom))
  
  if (countState.state === 'hasError') {
    return <Text>Error loading count: {countState.error.message}</Text>
  }
  
  if (countState.state === 'loading') {
    return <Text>Loading count...</Text>
  }
  
  return (
    <View>
      <Text>Count: {countState.data}</Text>
      <Button 
        title="Increment" 
        onPress={() => {
          setCount(async (promiseOrValue) => {
            try {
              return (await promiseOrValue) + 1
            } catch (err) {
              setError(err)
              return 0 // Fallback value
            }
...
4. **Performance Considerations**: AsyncStorage operations are asynchronous and can be slower than localStorage, so consider:
   - Batching updates when possible
   - Implementing caching for frequently accessed values
   - Using `loadable` to avoid suspense boundaries for better UX

5. **Testing AsyncStorage Atoms**: When testing components that use AsyncStorage atoms, make sure to:
   - Mock AsyncStorage in your tests
   - Account for the asynchronous nature of the storage operations
   - Test error cases and recovery

```jsx
// Example of mocking AsyncStorage in tests
