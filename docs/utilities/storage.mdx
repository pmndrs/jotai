...
**Important behavior in SPAs and React Native:** When using atomWithStorage in an SPA or React Native app, the default behavior (with `getOnInit` set to `false` or not specified) will **always return the initial value first**, even if a different value exists in storage.

This might be unexpected behavior, as the stored value will only be loaded after the initial render, causing the value to change. This happens even when using Suspense.

When `getOnInit` is set to `true`, the atom will always return the value directly from storage on initialization, avoiding the initial value → stored value change.
...
**Note** you probably want to set `getOnInit` to true if you're expecting the storage value to be available immediately instead of the default value
