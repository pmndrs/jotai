...
- **getOnInit** (optional, by default **false**): A boolean value indicating whether to get item from storage on initialization. Note that in an SPA and React Native with `getOnInit` either not set or `false` you will **always get the initial value** instead of the stored value on initialization. If the stored value is preferred set `getOnInit` to `true`.
