...
> In Jotai v3, `loadable` is no longer part of the core package.
> See the [v3 migration guide](../guides/migrating-to-v3.mdx#loadable-util)
> for a userland replacement built on `unwrap`.
> Alternatively, [`useAtomValueRaw`](../core/use-atom.mdx#useatomvalueraw) and
> [`useAtomValueRawSync`](../core/use-atom.mdx#useatomvaluerawsync) let you
> read an async atom's value without suspending.
