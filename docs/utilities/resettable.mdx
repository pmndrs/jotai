...
## Another implementation of atomWithDefault

### Look back to atomWithDefault behavior

As you can see above the example, the two atoms' relation is disconnected after updating created one, `count2Atom = atomWithDefault((get) => get(count1Atom) * 2)`.
...
const dataAtom = atom(
  (get) => {
    get(refreshCountAtom); // it's introduced at /docs/recipes/atom-creators#atomwithrefresh
