...
If a bunja needs creation-time data, use `bunja.withSeed`.
A default seed is required when the bunja is declared, so callers may still omit the seed.

The seed is used only when a matching bunja instance is first created.
It is not part of the bunja instance identity.
If the matching instance already exists, later seeds are ignored.
