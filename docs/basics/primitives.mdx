...
An atom in jotai represents a piece of state. It can hold any value ranging from string, boolean or number to complex structures like arrays, objects, functions and so on. You can consider them the building blocks of your app state.
...
## Store functions in atoms

We mentioned that our atoms can contain any kind of data, it can be a string, Blob, Observer, anything really. There is just one exception. Because read-only atoms are defined using a function, jotai will not understand if we give it a function that isn't exactly a pure getter.
