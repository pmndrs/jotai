...
const App = () => {
  return (
    <React.Suspense fallback={/* What to show while suspended */}>
...
**Note**: You cannot get the value of an async atom in a write-atom (wether its async or not) before its value has been set
...
Async write atoms are another kind of async atom. When the `write` function of atom returns a promise, triggering it will suspend the component.
...
const Component = () => {
  ...
  React.useEffect(() => {
    setTimeout(() => {
      increment() // triggers the suspend while `increment` is pending
    }, 2000)
  }, [])
}
```
...
Having more `<Suspense>`s in the component tree is also possible and must be considered to profit from jotai inherent handling at the best.
