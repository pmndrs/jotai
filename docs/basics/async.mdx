...
With async atoms, you gain access to real-world data but still managing them directly from your atoms and incredible ease.
...
Jotai is inherently leveraging `Suspense` to use asynchronous flows.
...
const Component = () => {
  ...
  if (value.state === 'pending') // 'pending' | 'hasData'
...
**Note**: You cannot use an async atom in a write-atom (wether its async or not) before it's value has been set
