...
When you compose atoms, prefer a wide graph over a deep one: an atom that derives from a few others is fine, but a long chain where each atom derives from the previous one is bounded by the JavaScript call stack and can overflow when read. See [dependency graph depth](performance.mdx#dependency-graph-depth) in the performance guide for details and how to flatten such chains.
