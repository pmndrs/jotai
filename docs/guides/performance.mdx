...
Using "atom config" as the initial value, changes in the atom within the component will not trigger re-rendering of other components. At the same time, `getAllAtom` can automatically synchronize all atom states without the need for additional status updates for the `togglesAtom` atom.

```tsx
const togglesAtom = atom([atom(false), atom(false), atom(false)])
const getAllAtom = atom(get => get(togglesAtom).map(get))
...
```tsx
const Item = ({ index, val }) => {
  const setToggles = useAtomValue(getAllAtom);
  const [toggle, setToggle] = useAtom(val)

  const onPress = () => {
    setToggle(!toggle) // Update locally
    console.log('All toggles', setToggles) // Will show the updated value
  }
}
```
