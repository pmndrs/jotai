...
As seen in the [core section](../api/core.mdx), React 18 default behaviour (and overall good practice) is to make sure your component functions are _idempotent_.
They will be called multiple times in the render phase, even at mount. So we need to keep our renders cheap at all cost!
...
Let's imagine you store in an atom an object that changes almost every second, it may not be best suited to "focus" on a specific properties of this object using `focusAtom`, because anyway they will all re-render in the same time, so best adding no overhead and not create any more atoms.
