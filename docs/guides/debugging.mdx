...
to inspect the state of a particular atom. Please refer to [`useAtomDevtools`](../api/devtools#use-atom-devtools) hook from `jotai/devtools` bundle.

## Frozen Atoms

To find bugs where you accidentally tried to mutate objects stored in atoms you
could use [`freezeAtom`](../api/utils#freeze-atom) or
[`atomFrozenInDev`](../api/utils#atom-frozen-in-dev) from `jotai/utils` bundle.
