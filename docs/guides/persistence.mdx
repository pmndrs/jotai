...
Jotai has an [atomWithStorage function in the utils bundle](../api/utils#atom-with-storage) for persistance that supports persisting state in `sessionStorage`, `localStorage`, `AsyncStorage`, or the URL hash.
...
This requires [onMount](../api/core#on-mount).
