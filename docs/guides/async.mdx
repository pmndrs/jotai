...
Alternatively, you could avoid the inherent suspending that Jotai does for you, by wrapping your atoms with the [`loadable` API](../utilities/async.mdx#loadable), or by reading the atom with [`useAtomValueRaw`](../core/use-atom.mdx#useatomvalueraw) instead of `useAtomValue`.
