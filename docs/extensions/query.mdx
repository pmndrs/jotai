...
⚠️ Note: If using **Typescript**, it is recommended to use a Map when passing the queryClient value to the useHydrateAtoms. You can find a working example in the [Initializing State on Render docs](https://jotai.org/docs/guides/initialize-atom-on-render#using-typescript)
