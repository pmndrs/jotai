...
[Bunja](https://github.com/disjukr/bunja) provides
a similar purpose and interface to "bunshi"
while maximizing the guarantee of maintaining the same reference
even under [React strict mode](https://github.com/saasquatch/bunshi/issues/58).

Useful in scenarios where maintaining the same object reference,
such as with WebSockets, is important.
...
import { useBunja, createScopeFromContext } from 'bunja/react'

const InitialCountContext = createContext({ initialCount: 0 })
const InitialCountScope = createScopeFromContext(InitialCountContext)

const countBunja = bunja([InitialCountScope], ({ initialCount }) => {
...
const App = () => (
  <div>
    <h1>With initial value 1</h1>
    <InitialCountContext value={{ initialCount: 0 }}>
      <Counter />
      <Counter />
    </InitialCountContext>
    <h1>With initial value 2</h1>
