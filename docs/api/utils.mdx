...
[`useResetAtom`](../api/utils#use-reset-atom) hook. It works exactly the same
way as primitive atom would, but you are also able to set it to a special value
[`RESET`](../api/utils#reset). See examples in [Resettable atoms](../guides/resettable).
...
Resets a [Resettable atom](../guides/resettable) to its initial value.
...
Special value that is accepted by [Resettable atoms](../guides/resettable)
created with [`atomWithReset`](../api/utils#atom-with-reset), [`atomWithDefault`](../api/utils#atom-with-default)
