...
The created atom config can have an optional property `debugLabel`. The debug label will be used to display the atom in debugging. See [Debugging guide](../guides/debugging) for more information.
...
Check [`atomFamily`](../api/utils#atom-family) in utils for parameterized atoms.
