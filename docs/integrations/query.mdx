...
- `atomsWithTanstackQuery` for [QueryObserver](https://tanstack.com/query/v4/docs/reference/QueryObserver)
- `atomsWithTanstackInfiniteQuery` for [InfiniteQueryObserver](https://tanstack.com/query/v4/docs/reference/InfiniteQueryObserver)
- `atomsWithTanstackMutation` for MutationObserver

All three functions follow the same signature.

```ts
const [dataAtom, statusAtom] = atomsWithTanstackSomething(
  getOptions,
  getQueryClient
)
```
...
## `atomsWithTanstackQuery` usage

`atomsWithTanstackQuery` creates new atoms that implement a standard [`Query`](https://tanstack.com/query/v4/docs/guides/queries) from TanStack Query.
...
`atomsWithTanstackInfiniteQuery` is very similar to `atomsWithTanstackQuery`, however it is for an `InfiniteQuery`, which is used for data that is meant to be paginated. You can [read more about Infinite Queries here](https://tanstack.com/query/v4/docs/guides/infinite-queries).
...
const idAtom = atom(1)
const [userAtom] = atomsWithTanstackInfiniteQuery((get) => ({
  queryKey: ['users', get(idAtom)],
  queryFn: async ({ queryKey: [, id] }) => {
    const res = await fetch(`https://jsonplaceholder.typicode.com/users/${id}`)
    return res.json()
...
`atomsWithTanstackMutation` creates new atoms that implement a standard [`Mutatioin`](https://tanstack.com/query/v4/docs/guides/mutations) from TanStack Query.
...
const idAtom = atom(1)
const [, postAtom] = atomsWithTanstackMutation((get) => ({
  mutationKey: ['posts'],
...
import { atomsWithTanstackQuery, queryClientAtom } from 'jotai-tanstack-query'
...
export const [todosAtom] = atomsWithTanstackQuery((get) => {
  return {
    queryKey: ['todos'],
    queryFn: () => fetch('/todos'),
  }
