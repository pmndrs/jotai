...
const Toggle: React.FC = () => {
  ...
  return (
    <div>
      {state.matches('reading') && <strong onDoubleClick={send}>{state.context.value}</strong>}
      ...
      <br />
      <div>
        Double-click to edit. Blur the input or press <code>enter</code> to commit. Press{' '}
