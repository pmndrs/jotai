...
For more info, please read this github discussion https://github.com/reactwg/react-18/discussions/41 under How is it different from setTimeout?
...
export default function atomWithDebounce<T>(
  ...
  const prevTimeoutAtom = atom<ReturnType<typeof setTimeout> | null>(null)
  ...
  const clearTimeoutId = (
    get: Getter,
    set: Setter,
    prevTimeoutAtom: PrimitiveAtom<ReturnType<typeof setTimeout> | null>
  ) => {
  ...
  const clearTimeoutAtom = atom(null, (get, set, _arg) => {
    clearTimeoutId(get, set, prevTimeoutAtom)
    set(isDebouncingAtom, false)
    set(_currentValueAtom, get(debouncedValueAtom))
  })
