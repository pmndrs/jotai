...
create a component or iframes) and workers on the same origin.
...
const atomBroad = atomWithBroadcast("count", 0);

const ListOfThings: FC = () => {
  const [count, setCount] = useAtom(atomBroad);
