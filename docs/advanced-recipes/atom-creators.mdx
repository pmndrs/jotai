...
This can be especially useful for text search inputs, where you would like to fire an action only after the user has stopped typing for a delay, instead of firing an action on every keystroke.
...
export function atomWithDebounce<Value>(
  ...
  const isDebouncingAtom = atom(
    (get) => get(currentValueAtom) !== get(debouncedValueAtom)
  )

  return {
    readOnlyCurrentValueAtom: atom((get) => get(currentValueAtom)),
    isDebouncingAtom,
    debouncedValueAtom,
  }
}
...
The sandbox link below shows how we would use a derived atom to fetch state based on the value of `debouncedValue`.

When typing a pokemon's name in `<SearchInput>`, we do not fire a fetch on every letter, but only after `delayMilliseconds` has passed since the last text input.

[CodeSandbox](https://codesandbox.io/s/withered-tdd-cjrz2y?file=/src/App.tsx)
