...
const withSwappedInternals = (experimentId, fn) => {
  ...
  if (!fs.existsSync(experiment.internalsPath)) {
    throw new Error(
      `Experiment internals file not found: ${experiment.internalsPath}`,
    )
  }
  ...
  try {
    ...
    if (restoredInternals !== originalInternals) {
      throw new Error(
        `Failed to restore root internals.ts after experiment ${experimentId}`,
      )
    }
  }
}
...
const snapshotExperimentDist = (experimentId) => {
  ...
  if (!fs.existsSync(path.join(experimentDistPath, 'vanilla.js'))) {
    throw new Error(
      `Experiment dist snapshot missing vanilla.js: ${experimentDistPath}`,
    )
  }
...
const entryFromToken = (versionToken) => {
  ...
  if (EXPERIMENT_BY_ID[idCandidate]) {
    ...
    if (!fs.existsSync(exp.internalsPath)) {
      throw new Error(
        `Experiment internals file not found: ${exp.internalsPath}`,
      )
    }
    return {
      label: exp.label,
...
const main = async () => {
  if (showHelp) {
    ...
    console.log('')
    console.log('Usage:')
    console.log('  pnpm run test:bench --iterations=7 --warmup=5')
    console.log(
    ...
    console.log(
      'HEAD(dist) and experiments are built first and benchmarked from local dist artifacts.',
    )
    console.log(
      'Experiment builds pass JOTAI_BENCH_SWAP_INTERNALS=<id>, swap src/vanilla/internals.ts, and snapshot to dist/bench-experiments/<id>.',
    )
    console.log('')
    ...
    console.log(
      '  Fastest value in each scenario is marked as (base) and omits percentage.',
    )
    console.log('  Rows without a scenario base show only <median_ms>.')
    console.log(
      '  Cell colors are based on predecessor performance deltas (positive=faster): <= -30% red, >= +30% green.',
    )
    console.log('')
    ...
    const legendLine = (label, style, description) =>
      console.log(
        `  ${color(label.padEnd(legendLabelWidth), ...style)}  ${description}`,
      )
    legendLine(
      'column fastest',
      [ANSI.bold, ANSI.yellow],
      'Fastest value in that scenario column (base).',
    )
    legendLine(
      '<= -30% vs predecessor',
      [ANSI.bold, ANSI.red],
      'Performance delta vs predecessor is -30% or lower (slower).',
    )
    legendLine(
      '>= +30% vs predecessor',
      [ANSI.bold, ANSI.green],
      'Performance delta vs predecessor is +30% or higher (faster).',
    )
    return
  }
  ...
  if (workerJsonMode) {
    ...
    if (workerEntries.length !== 1) {
      throw new Error(
        `Worker mode expects exactly one version token, got ${workerEntries.length}.`,
      )
    }
    ...
    if (entry.source === 'local') {
      ...
      const experimentDistPath = ensureLocalBuild({
        repoPath: entry.repoPath,
        experimentId: entry.id,
      })
      one = runOne(entry.label, entry.repoPath, true, experimentDistPath)
    } else {
      one = withDownloadedVersion(entry.label, (pkgPath) =>
        runOne(entry.label, pkgPath),
      )
    }
  ...
  const runEntryInChild = async (e) => {
    ...
    const token =
      e.source === 'local' ? 'HEAD' : e.source === 'experiment' ? e.id : e.label
  ...
  const predecessorByLabel = Object.fromEntries(
    orderedLabels.map((label, idx) => [
      label,
      idx > 0 ? orderedLabels[idx - 1] : null,
    ]),
  )
      ...
      if (value != null && fastest != null && value === fastest) {
        ...
        const predecessorValue =
          predecessorLabel == null
            ? null
            : allByLabel[predecessorLabel]?.results?.[s]?.medianMs
        if (predecessorValue != null && predecessorValue !== 0) {
          ...
          const predecessorDeltaPct =
            ((predecessorValue - value) / predecessorValue) * 100
          if (predecessorDeltaPct <= -30)
            text = color(text, ANSI.bold, ANSI.red)
          else if (predecessorDeltaPct >= 30)
            text = color(text, ANSI.bold, ANSI.green)
        }
      }
      cells.push(text)
    }
  ...
  if (threshold != null && head) {
    ...
    if (!thresholdBaselineRow) {
      throw new Error(
        'Threshold check requires the last selected version result.',
      )
    }
    const breaches = []
    for (const s of scenarios) {
      ...
      if (headValue == null || baselineValue == null || baselineValue === 0)
        continue
