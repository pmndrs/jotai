...
it('second useAtom call should not return old data (#1127)', async () => {
  const testAtom = atom('foo')

  const ComponentWithAtoms = () => {
    const [value, setValue] = useAtom(testAtom)
    const [value2] = useAtom(testAtom)
    const [rows, setRows] = useState<string[]>([])
    const commits = useCommitCount()

    useEffect(() => {
      const row = `${value}, ${value2}`
  ...
  if (!IS_REACT18) {
    ...
    expect(tearing).toBeFalsy()
  }
  await findByText('bar, bar')
})
