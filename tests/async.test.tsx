...
// Delete the following two tests later.
// They don'twork because promise atom won't be mounted until the promise is resolved.

it('should remount promise atom with onMount', async () => {
  const countAtom = atom(
    new Promise<number>((r) => setTimeout(() => r(-1), 3600 * 1000))
  )
  countAtom.onMount = (setCount) => {
    setTimeout(() => {
      setCount(Promise.resolve(1))
    }, 10)
  }
  const resetCountAtom = atom(null, (_get, set) => {
    set(
      countAtom,
      new Promise<number>((r) => setTimeout(() => r(-1), 3600 * 1000))
    )
...
it('should remount promise atom with intermediate atom (#500)', async () => {
